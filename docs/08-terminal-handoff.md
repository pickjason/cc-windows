...
同时读取 `#{pane_width} #{pane_height}`,把快照通过 `term_snapshot {data,cols,rows}` 推给网页。网页渲染前必须先把 xterm resize 到 tmux pane 的真实 `cols/rows`,再 `\x1b[2J\x1b[H`(清屏回原点)+ 快照内容。只读、有 ~0.5s 级延迟、不可交互——「看本地在干啥」足够。

这个尺寸同步很关键:本地 Terminal 可能是 200+ 列,网页 dock 可能只有 100 多列。若直接把 200+ 列的 `capture-pane` 快照写进较窄 xterm,Claude 的选择器/状态栏会被重新换行,出现选项重复、残字和光标错位。
...
| `term_snapshot` | `{ sessionId, data, cols, rows }` | READONLY 态周期推送的整屏快照;前端按 tmux pane 尺寸 resize 后清屏写入 |
...
- `TerminalPane` 收 `term_mode` 维护 `readonly` 标志:
  ...
  - 收 `term_snapshot` → `term.resize(cols, rows)` → `term.write("\x1b[2J\x1b[H" + data)`。
...
| 服务重启时本地终端已经 attach | `discover()` 按当前 client 数初始化为 READONLY;网页 attach 前再同步一次,避免误建 web attach 抢尺寸 |
